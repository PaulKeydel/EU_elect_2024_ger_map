...
df_kw24 = rearrange_KomW_data.create_df_KomW24()
...
for _, row in dfgeo.iterrows():
    ...
    if (df_kw24["AGS"] == ags).any():
        valid_votes["KOM24"].append(df_kw24["Gültige"].loc[df_kw24["AGS"] == ags].item())
        abs_votes_linke["KOM24"].append(df_kw24["LINKE"].loc[df_kw24["AGS"] == ags].item())
        electorate["KOM24"].append(df_kw24["Wahlberechtigte"].loc[df_kw24["AGS"] == ags].item())
        num_voters["KOM24"].append(df_kw24["Wählende"].loc[df_kw24["AGS"] == ags].item())
    else:
...
print("Wahlbeteiligung bundesweit:")
print("  EU2019: " + f"{elect_turnout_19:.2f}" + "%")
print("  EU2024: " + f"{elect_turnout_24:.2f}" + "%")
print("DIE LINKE - Stimmendifferenzen zwischen EU24 und EU19 in den Landkreisen:")
...
print("DIE LINKE - Wahlverhalten Kommunalwahl / Europawahl")
cosine_sim = 1 - cosine(dfgeo["results_linke_kom"].loc[~dfgeo["results_linke_kom"].isna()], dfgeo["results_linke_24"].loc[~dfgeo["results_linke_kom"].isna()])
print("  cosine similarity: " + f"{cosine_sim:.3f}")
